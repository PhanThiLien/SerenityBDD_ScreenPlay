...
Feature: Add new todos
  ...
  Scenario: Adding items to several peoples lists
    Given that 'James' has a todo list containing:
      | Buy some milk |
      | Walk the dog  |
    And that 'Jill' has a todo list containing:
      | Buy some milk   |
      | Buy some cheese |
    When she adds 'Buy some cereal' to her list
    Then Jill's todo list should contain:
      | Buy some milk   |
      | Buy some cheese |
      | Buy some cereal |
    And James's todo list should contain:
      | Buy some milk |
      | Walk the dog  |
